...
def test_stack_archive_and_restore_lifecycle(data_dir):
    """UC611 & UC612: Archive stack and restore stack."""
    from services import cloud_provisioning
    ...
    # Archive stack
    res_arch = cloud_provisioning.archive_stack(proj, stk, actor="ops_user", reason="Decommissioned project")
    assert res_arch["ok"] is True
    assert res_arch["archived"] is True

    m = cloud_provisioning._load_meta(proj, stk)
    assert m["archived"] is True
    assert m["archive_reason"] == "Decommissioned project"

    # Restore stack
    res_rest = cloud_provisioning.restore_archived_stack(proj, stk, actor="ops_lead")
    assert res_rest["ok"] is True
    assert res_rest["archived"] is False

    m2 = cloud_provisioning._load_meta(proj, stk)
    assert m2["archived"] is False
    assert m2["restored_by"] == "ops_lead"


def test_api_archive_restore_endpoints(data_dir):
    """UC611 & UC612: POST archive, POST restore, GET archived."""
    from pathlib import Path
    from auth.service import generate_token
    from services.cloud_provisioning import bp
    from storage import pg
    ...
    org_id = "org-arch"
    proj_id = "proj-arch-api"
    pg.execute("INSERT INTO orgs (id, name, created_at) VALUES (%s, %s, %s) ON CONFLICT DO NOTHING", (org_id, "Arch Org", 1000))
    pg.execute("INSERT INTO projects (id, name, org_id, created_at) VALUES (%s, %s, %s, %s) ON CONFLICT DO NOTHING", (proj_id, "Arch Proj", org_id, 1000))
    pg.execute("INSERT INTO org_members (org_id, user_id, role, created_at) VALUES (%s, %s, %s, %s) ON CONFLICT DO NOTHING", (org_id, "u1", "admin", 1000))
    ...
    resp_arch = client.post(
        "/api/cloud-provisioning/stacks/old-stack/archive",
        json={"reason": "cleanup"},
        headers=headers,
    )
    assert resp_arch.status_code == 200
    assert resp_arch.get_json()["archived"] is True

    # 2. List archived
    resp_list = client.get("/api/cloud-provisioning/stacks/archived", headers=headers)
    assert resp_list.status_code == 200

    # 3. Restore
    resp_rest = client.post(
        "/api/cloud-provisioning/stacks/old-stack/restore",
        json={},
        headers=headers,
    )
    assert resp_rest.status_code == 200
    assert resp_rest.get_json()["archived"] is False
