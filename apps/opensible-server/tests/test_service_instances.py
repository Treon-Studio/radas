...
def test_project_isolation_and_project_derived_org_authorization(pg_db):
    ...
    created = _instance()
    assert service_instances.get_instance("project-b", created["id"]) is None
    assert service_instances.list_instances("project-b") == []
    with pytest.raises(service_instances.ProjectAuthorizationError):
        _instance(org_id="org-b")
    with pytest.raises(service_instances.ProjectAuthorizationError):
...
def test_revision_is_immutable_snapshot_and_desired_is_separate(pg_db):
    ...
    original = service_instances.get_revision("project-a", instance["id"], revision_number=1)
    assert original["revision_number"] == 1
    spec["resources"]["memory_mb"] = 2048
    revision = service_instances.create_revision(instance["id"], spec, "owner", project_id="project-a")
    assert revision["revision_number"] == 2
    assert service_instances.get_revision("project-a", instance["id"], revision_number=1)["spec"]["resources"]["memory_mb"] == 512
    current = service_instances.get_instance("project-a", instance["id"])
    assert current["desired_revision_id"] == revision["id"]
    assert service_instances.get_revision("project-a", instance["id"], revision_id=original["id"])["id"] == original["id"]
...
def test_valid_and_invalid_instance_state_transitions_and_cas(pg_db):
    ...
    running = service_instances.update_observed_status(
        instance["id"], "provisioning", project_id="project-a", expected_status="draft",
    ...
    running = service_instances.update_observed_status(
        ...
        endpoint_summary={"url": "https://example.test"},
    )
    assert running["status"] == "running"
    with pytest.raises(service_instances.InvalidInstanceState):
        service_instances.update_observed_status(instance["id"], "destroyed", project_id="project-a")
    with pytest.raises(service_instances.InstanceConflictError):
        service_instances.update_observed_status(
            instance["id"], "degraded", project_id="project-a", expected_status="provisioning"
        )
...
def test_concurrent_operation_creators_share_one_idempotent_row(pg_db):
    ...
    def create() -> None:
        try:
            ...
            results.append(service_operations.create_operation("project-a", "service.deploy", "same-key", {"revision_id": "r1"}))
...
def test_specs_and_provider_outputs_never_persist_secret_values(pg_db):
    ...
    revision = service_instances.get_revision("project-a", instance["id"], revision_number=1)
    assert "raw-password" not in str(revision)
    assert "raw-key" not in str(revision)
    service_instances.update_observed_status(
        instance["id"], "provisioning", project_id="project-a",
