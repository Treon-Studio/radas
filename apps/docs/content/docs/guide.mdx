...
Here is a quick overview of how to get started with radas. In this guide, we will:
...
After [downloading](https://app.radas.com/downloads) and installing radas, you will be greeted with the welcome screen. From here you can import or clone a repository.

<ImageSection
  alt="Import screen"
  src="/started/welcome-screen.png"
  subtitle="First thing you see when you launch radas"
/>
...
The first step is to choose your "target" branch. radas focuses on a trunk based development model, so first you need to tell us what branch means "production" to you. Normally this is something like `origin/master` or `origin/main`.

If you would like to use our AI features like commit message generation, you can also sign up for a radas account and log in. This is optional, but we'll do it here so we can show off these features.

You can also connect radas to your GitHub account so we can automatically open Pull Requests for you, but this is also optional.
...
Now we can start working. radas will watch your working directory and immediately show any changes that you make.
...
radas sees that our target branch has new work and shows us the "Update" button, which we can click to rebase our feature branch on top of the new work and also remove the README branch automatically, since we can see that it has been integrated upstream.
...
radas makes it very easy to edit, reorder, squash and split commits. If you hover over any commit, you will see a drag handle on the top right. You can drag the commit to reorder it, or drag it on top of another commit to squash them together.
