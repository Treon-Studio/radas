...
GitHub and GitLab provide a mechanism to verify signed commits using an uploaded public SSH or GPG key. radas can be configured automatically sign all your commits.
...
1. Tell radas to sign your commits
2. Upload your key as a "signing key" to GitHub or GitLab (or elsewhere)

## Telling radas to Sign

For radas to sign commits, you need to setup Git to sign commits, as we do roughly the same thing that Git itself tries to do, and we read and respect most of the same Git config settings.

The main difference is that instead of only the `commit.gpgSign` as the flag that tells Git to automatically sign commits, we look for `radas.signCommits` first. Thus, if Git would sign, radas will attempt to sign your commits with the normal Git settings as well.
But if something goes wrong, `radas.signCommits` will be set to `false` in the repository-local settings to prevent commits from
failing generally.
...
$ git config --global radas.signCommits true
```
...
<Callout title="Using radas's Generated SSH Key">
Earlier versions of radas would only sign with it's generated SSH key. Although we've removed that functionality, you can easily set it back up by pointing the signingKey at the generated SSH Key. The key is located in the following locations:

<Tabs groupId="platform" items={['macOS', 'Windows', 'Linux']} persist>
  <Tab value="macOS">
  ```bash
  /Users/[username]/Library/Application Support/com.radas.app/keys/ed25519.pub
  ```
  </Tab>
  <Tab value="Windows">
  ```bash
  C:\Users\[username]\AppData\Roaming\com.radas.app\keys\ed25519.pub
  ```
  </Tab>
  <Tab value="Linux">
  ```bash
  ~/.local/share/radas-tauri/keys/ed25519.pub
  ```
  </Tab>
</Tabs>
...
Now paste in the public SSH key you copied from radas, name it and make sure the "Usage Type" is either "Signing" or "Authentication and Signing".

<ImageSection
  alt="Adding a key to GitHub"
  src="/signing-06.png"
  subtitle="Now all your radas generated commits will be verified on that platform!"
/>
