...
Butler Flow is a lightweight, branch-based workflow enabled by radas virtual branch functionality.
...
In radas, virtual branches will not work without the specification of a special "target branch". Everything exists in relation to this special branch, everything that differs from it must be accounted for by being owned by some other branch, until those changes are integrated.

### Virtual Branches

Once you choose a target branch, everything that is in your working directory and not ignored by the Git ignore specification must be owned by a virtual branch. If you don't have one active, radas will automatically create one for you.
...
All coworkers' work, whether created and managed by radas or not, exists on the central server as normal Git branches. These will automatically be pulled down and kept up to date by radas and can be converted into virtual branches and applied to your working directory in addition to your branches.
