...
radas allows you to create an ordered stack of branches where each branch depends on (and is based on) the previous one.
...
In fact radas implements stacked branches as Virtual Branches that are split into multiple dependent branches.
...
the radas will be able to create a slightly better experience if the "Merge" strategy is used.
