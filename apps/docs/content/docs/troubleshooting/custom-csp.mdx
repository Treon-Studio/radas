...
You can add a custom CSP by editing the radas configuration file, found at the following location:

<Tabs groupId="platform" items={['macOS', 'Windows', 'Linux']} persist>
  <Tab value="macOS">
  ```bash
  ~/Library/Application\ Support/radas/settings.json
  ```
  </Tab>
  <Tab value="Windows">
  ```bash
  C:\Users\[username]\AppData\Roaming\radas\settings.json
  ```
  </Tab>
  <Tab value="Linux">
  ```bash
  ~/.config/radas/settings.json
  ```
  </Tab>
</Tabs>
...
The changes will take effect the next time you start radas.