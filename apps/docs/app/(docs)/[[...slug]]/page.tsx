...
export default async function Page(props: { params: Promise<Param> }): Promise<React.ReactElement> {
  ...
  const footer = (
    <>
      <a
        href={`https://github.com/radasapp/radas-docs/blob/main/content/docs/${page.file.path}`}
        target="_blank"
      ...
      <a
        href={`https://github.com/radasapp/radas-docs/issues/new?label=docs&title=Feedback+for+page+"${page.file.flattenedPath}"`}
        target="_blank"
        rel="noreferrer noopener"
        className="group flex items-center justify-center gap-2 rounded-md border border-neutral-300/50 py-1 text-sm text-neutral-500 transition duration-300 hover:bg-neutral-100 dark:border-neutral-700 dark:bg-neutral-900 dark:text-neutral-400 dark:hover:bg-neutral-950"
      >
