...
import { RiCheckLine as Check, RiCloseLine as Close, RiLoader4Line as Loader, RiStopCircleLine as Stop, RiRefreshLine as Retry } from "@remixicon/react";
...
export function ServiceOperationPanel({ projectId, serviceId, operation: initial, onChanged, onRetry }: Props) {
    ...
    const poll = async () => {
      try {
        ...
        try {
          ...
          if (!stopped) setEvents(readEvents(eventResult));
        } catch (eventError) {
          eventFailed = true;
          if (!stopped) setPollError(eventError);
        }
        if (stopped) return;
        if (!eventFailed) setPollError(null);
  ...
  const timestampLabel = (value?: string | number) => epochDate(value)?.toLocaleString("id-ID") || "—";
  return <Card><CardHeader className="pb-3"><div className="flex items-center justify-between gap-3"><CardTitle className="text-sm">Operasi terbaru</CardTitle><Badge variant={statusToVariant(status)}>{status}</Badge></div><p className="text-xs text-[var(--color-muted-foreground)]">{operation.kind || "service operation"} · {progressLabel}</p></CardHeader><CardContent className="space-y-4 pt-0"><div className="flex items-center gap-2 text-sm" role="status" aria-live="polite">{active ? <Loader className="h-4 w-4 animate-spin" /> : status === "succeeded" ? <Check className="h-4 w-4 text-[var(--color-success)]" /> : <Close className="h-4 w-4 text-[var(--color-destructive)]" />}<span>{String(operation.error_message || progressLabel)}</span></div>{active && <Button type="button" size="sm" variant="outline" onClick={() => void cancel()} disabled={canceling}><Stop className="h-3.5 w-3.5" /> {canceling ? "Membatalkan…" : "Batalkan operasi"}</Button>}{status === "failed" && operation.retryable && onRetry && <Button type="button" size="sm" variant="outline" onClick={() => onRetry(operation)}><Retry className="h-3.5 w-3.5" /> Coba lagi</Button>}{(endpointValue || hasHealth) && <div className="grid gap-2 rounded-md bg-[var(--color-muted)]/50 p-3 text-xs sm:grid-cols-2"><div><span className="text-[var(--color-muted-foreground)]">Endpoint</span><div className="mt-1 break-all font-medium">{endpointLabel}</div></div><div><span className="text-[var(--color-muted-foreground)]">Health</span><div className="mt-1 font-medium">{healthLabel}</div></div></div>}<div className="grid gap-2 text-xs sm:grid-cols-3"><div><span className="text-[var(--color-muted-foreground)]">Created</span><div className="mt-1">{timestampLabel(operation.created_at)}</div></div><div><span className="text-[var(--color-muted-foreground)]">Started</span><div className="mt-1">{timestampLabel(operation.started_at)}</div></div><div><span className="text-[var(--color-muted-foreground)]">Finished</span><div className="mt-1">{timestampLabel(operation.finished_at)}</div></div></div>{operation.poll_url && <a className="break-all text-xs text-[var(--color-primary)] underline" href={operation.poll_url}>Poll URL: {operation.poll_url}</a>}{pollError != null && !isForbidden(pollError) && <p className="text-xs text-[var(--color-destructive)]">Pembaruan status gagal sementara; mencoba lagi.</p>}{events.length > 0 ? <div className="max-h-48 space-y-2 overflow-auto rounded-md bg-[var(--color-muted)]/50 p-3 font-mono text-xs">{events.map((event, index) => <div key={`${event.created_at || "event"}-${index}`} className="flex gap-2"><span className="text-[var(--color-muted-foreground)]">{epochDate(event.created_at)?.toLocaleTimeString("id-ID") || ""}</span><span>{String(event.message || event.event || "Event")}</span></div>)}</div> : <p className="text-xs text-[var(--color-muted-foreground)]">Belum ada event operasi.</p>}</CardContent></Card>;
}
