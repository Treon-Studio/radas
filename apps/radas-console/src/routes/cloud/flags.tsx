...
function directDependentCount(flags: FlagType[], key: string): number { return flags.filter((flag) => flag.parent_key === key || (flag.prerequisites || []).includes(key)).length; }
function relationshipWarning(flag: FlagType, flags: FlagType[]): string | null {
  const related = [flag.parent_key, ...(flag.prerequisites || [])].filter(Boolean) as string[];
  const blocked = related.map((key) => flags.find((candidate) => candidate.key === key)).find((candidate) => candidate && (!candidate.enabled || candidate.kill_switch || candidate.archived));
  if (!blocked) return null;
  return blocked.archived ? `${blocked.key} diarsipkan` : blocked.kill_switch ? `${blocked.key} dihentikan darurat` : `${blocked.key} nonaktif`;
}
...
function FlagsPage() {
  ...
  const [archiveKey, setArchiveKey] = useState<string | null>(null); const [deleteKey, setDeleteKey] = useState<string | null>(null); const [deleteTypedKey, setDeleteTypedKey] = useState(""); const [deleteConfirmKey, setDeleteConfirmKey] = useState<string | null>(null); const [rollbackKey, setRollbackKey] = useState<string | null>(null); const [killPending, setKillPending] = useState<string | null>(null);
  ...
  const toggleMut = useMutation({
    ...
    onMutate: async ({ k, patch }) => { await qc.cancelQueries({ queryKey: ["flags"] }); const previous = qc.getQueryData<{ flags: FlagType[] }>(["flags"]); const previousSelected = selected?.key === k ? selected : null; qc.setQueryData<{ flags: FlagType[] }>(["flags"], (old) => old ? { flags: old.flags.map((flag) => flag.key === k ? { ...flag, ...patch } : flag) } : old); setSelected((current) => current?.key === k ? { ...current, ...patch } : current); return { previous, previousSelected }; },
  ...
  const relationshipMut = useMutation({
    mutationFn: ({ flagKey, parent_key, prerequisites }: { flagKey: string; parent_key: string | null; prerequisites: string[] }) => api<{ success: boolean; flag: FlagType }>("PATCH", `/api/flags/${encodeURIComponent(flagKey)}`, { parent_key, prerequisites }),
  ...
  const archiveMut = useMutation({
    mutationFn: (flagKey: string) => api<{ success: boolean; flag: FlagType }>("POST", `/api/flags/${encodeURIComponent(flagKey)}/archive`, {}),
    onSuccess: () => { setArchiveKey(null); setSelected(null); toast.success("Flag diarsipkan dan dinonaktifkan"); },
    onError: (error: unknown) => toast.error(localizedMutationError(error, "Gagal mengarsipkan flag")),
    onSettled: () => { invalidate(); invalidateImpact(); },
  });
  const restoreMut = useMutation({
    mutationFn: (flagKey: string) => api<{ success: boolean; flag: FlagType }>("POST", `/api/flags/${encodeURIComponent(flagKey)}/restore`, {}),
  ...
  const deleteMut = useMutation({
    mutationFn: (flagKey: string) => api("DELETE", `/api/flags/${encodeURIComponent(flagKey)}`),
    onSuccess: () => { setDeleteConfirmKey(null); setDeleteKey(null); setDeleteTypedKey(""); setSelected(null); toast.success("Flag dihapus permanen"); },
    onError: (error: unknown) => toast.error(localizedMutationError(error, "Gagal menghapus flag")),
    onSettled: () => { invalidate(); invalidateImpact(); },
  });
  const rollbackMut = useMutation({ mutationFn: (flagKey: string) => api("POST", `/api/flags/${encodeURIComponent(flagKey)}/rollback`), onSuccess: () => { invalidate(); invalidateImpact(); setRollbackKey(null); setSelected(null); toast.success("Konfigurasi dikembalikan"); }, onError: (error: unknown) => toast.error(localizedMutationError(error, "Gagal mengembalikan konfigurasi flag")) });
  const previewMut = useMutation({ mutationFn: (input: { key: string; env: string; user: string }) => api<EvaluationResult>("POST", "/api/flags/evaluate", input), onSuccess: (result) => setPreviewResult(result), onError: (error: unknown) => toast.error(localizedMutationError(error, "Gagal mengevaluasi flag")) });
  ...
  const flagAuditQuery = useQuery({ queryKey: ["flag-audit", selected?.key], queryFn: () => api<{ audit: any[] }>("GET", `/api/flags/audit?limit=500&flag_key=${encodeURIComponent(selected!.key)}`), enabled: !!selected && panelTab === "audit" });
  const impactQuery = useQuery({ queryKey: ["flag-impact", selected?.key], queryFn: () => api<FlagImpact>("GET", `/api/flags/${encodeURIComponent(selected!.key)}/impact`), enabled: !!selected });

  const openFlagByKey = (flagKey: string) => { const target = flags.find((flag) => flag.key === flagKey); if (target) openFlag(target); };
  ...
  return <div className="space-y-4">
    ...
    <div className="grid gap-3 md:grid-cols-2">{namespaces.map((namespace) => <div key={namespace} className="contents"><div className="md:col-span-2 text-xs font-mono uppercase tracking-wider text-[var(--color-muted-foreground)]">{namespace}</div>{visibleFlags.filter((flag) => (flag.namespace || "default") === namespace).map((flag) => { const warning = relationshipWarning(flag, flags); const dependents = directDependentCount(flags, flag.key); return <Card key={flag.id || `${flag.scope_type}-${flag.scope_id}-${flag.key}`} className="transition-colors hover:border-[var(--color-primary)]/50"><CardHeader className="py-3"><CardTitle className="flex items-center gap-2 text-sm"><button type="button" className="inline-flex min-w-0 items-center gap-2 text-left" onClick={() => openFlag(flag)} aria-label={`Lihat konfigurasi ${flag.key}`}><Flag className="h-4 w-4" /><code className="font-mono">{flag.key}</code><Badge variant={statusLabel(flag) === "Aktif" ? "success" : statusLabel(flag) === "Diarsipkan" ? "warning" : "destructive"}>{statusLabel(flag)}</Badge>{(flag.rollout_percent ?? 100) < 100 && <Badge variant="warning">{flag.rollout_percent}%</Badge>}</button><span className="ml-auto" onClick={(event) => event.stopPropagation()}><Switch checked={flag.enabled} disabled={!!flag.kill_switch || !!flag.archived} title={flag.archived ? "Flag diarsipkan" : flag.kill_switch ? "Hentikan darurat aktif" : undefined} aria-label={`Ubah status ${flag.key}`} onChange={(value) => toggleMut.mutate({ k: flag.key, patch: { enabled: value } })} /></span></CardTitle></CardHeader><CardContent className="space-y-2 pt-0 text-sm"><div className="text-[var(--color-muted-foreground)]">{flag.description || "—"}</div><div className="flex flex-wrap items-center gap-1 text-[11px]"><Badge>{flag.type || "release"}</Badge><Badge>{scopeLabel(flag.scope_type, flag.scope_id, flag.scope_name, flag.project_name, flag.organization_name)}</Badge>{flag.parent_key && <Badge>Induk: {flag.parent_key}</Badge>}{(flag.prerequisites || []).length > 0 && <Badge>Butuh {flag.prerequisites!.length} prasyarat</Badge>}{dependents > 0 && <Badge>Dipakai oleh {dependents}</Badge>}<button type="button" className="ml-auto inline-flex items-center gap-1 font-medium text-[var(--color-primary)] hover:underline" onClick={() => openFlag(flag)}>Lihat konfigurasi <ChevronRight className="h-3.5 w-3.5" /></button></div>{warning && <div className="flex items-center gap-1 text-[11px] text-[var(--color-warning)]"><Warning className="h-3.5 w-3.5" /> Tertahan: {warning}</div>}<div className="flex flex-wrap gap-1">{ENVS.map((env) => <span key={env} className={`rounded-full border px-2 py-0.5 text-[11px] ${flag.environments?.[env] ? "border-[var(--color-success)]/40 bg-[var(--color-success)]/10" : "opacity-40"}`}>{env}</span>)}</div>{(flag.tags || []).length > 0 && <div className="flex gap-1">{flag.tags!.map((tag) => <Badge key={tag}>{tag}</Badge>)}</div>}</CardContent></Card>; })}</div>)}</div>

    <Drawer open={!!selected} onClose={() => setSelected(null)} ariaLabel={selected ? `Konfigurasi ${selected.key}` : "Konfigurasi feature flag"} title={selected ? <span className="flex items-center gap-2"><code className="font-mono">{selected.key}</code><Badge variant={statusLabel(selected) === "Aktif" ? "success" : statusLabel(selected) === "Diarsipkan" ? "warning" : "destructive"}>{statusLabel(selected)}</Badge></span> : "Konfigurasi"} footer={selected && <div className="flex flex-wrap items-center gap-x-5 gap-y-2"><label className="flex items-center gap-2 text-sm"><Switch checked={selected.enabled} disabled={!!selected.kill_switch || !!selected.archived} title={selected.archived ? "Pulihkan dari arsip terlebih dahulu" : selected.kill_switch ? "Hentikan darurat aktif" : undefined} aria-label={`Ubah status ${selected.key}`} onChange={(value) => toggleMut.mutate({ k: selected.key, patch: { enabled: value } })} />{statusLabel(selected)}</label><Button size="sm" variant="outline" onClick={() => setRollbackKey(selected.key)} disabled={rollbackMut.isPending || !!selected.archived}><Refresh className="h-3.5 w-3.5" /> Kembalikan konfigurasi</Button></div>}>
      {selected && <div className="space-y-4"><p className="text-sm text-[var(--color-muted-foreground)]">{selected.description || "—"}</p><div className="flex flex-wrap gap-1"><Badge>{selected.type || "release"}</Badge><Badge>{scopeLabel(selected.scope_type, selected.scope_id, selected.scope_name, selected.project_name, selected.organization_name)}</Badge>{selected.archived && <Badge variant="warning">Tidak aktif dari arsip</Badge>}</div><Tabs<PanelTab> tabs={[{ id: "details", label: "Konfigurasi" }, { id: "audit", label: "Riwayat perubahan" }, { id: "preview", label: "Evaluasi" }]} active={panelTab} onChange={setPanelTab} />
        {panelTab === "details" && <div className="space-y-4 pt-3"><div><div className="mb-1.5 text-xs font-medium text-[var(--color-muted-foreground)]">Environment</div><p className="mb-2 text-[11px] text-[var(--color-muted-foreground)]">Pilih environment tempat flag aktif. Hentikan darurat mengesampingkan pilihan ini.</p><div className="grid gap-1.5">{ENVS.map((env) => <div key={env} className="flex items-center justify-between rounded-md border border-[var(--color-border)] px-3 py-1.5 text-sm"><span>{env}</span><Switch checked={selected.environments?.[env] === true} disabled={!!selected.archived} aria-label={`Ubah environment ${env}`} onChange={(value) => toggleMut.mutate({ k: selected.key, patch: { environments: { ...(selected.environments || {}), [env]: value } } })} /></div>)}</div></div><Field label="Persentase rollout" help="Persentase pengguna yang menerima flag secara bertahap."><Input type="number" min={0} max={100} disabled={!!selected.archived} value={selected.rollout_percent ?? 100} onChange={(event) => toggleMut.mutate({ k: selected.key, patch: { rollout_percent: Number(event.target.value) } })} /></Field><Field label="Masa berlaku (detik)">{selected.ttl_seconds ?? "—"}</Field><Field label="Berakhir otomatis pada">{selected.scheduled_expire_at ? new Date(selected.scheduled_expire_at * 1000).toLocaleString("id-ID") : "—"}</Field><Field label="User yang selalu diizinkan">{(selected.users_whitelist || []).length ? selected.users_whitelist!.join(", ") : "—"}</Field><Field label="User yang selalu diblokir">{(selected.users_blacklist || []).length ? selected.users_blacklist!.join(", ") : "—"}</Field>
          <section aria-labelledby="relationship-heading" className="rounded-md border border-[var(--color-border)] p-3"><div className="flex items-center justify-between gap-3"><div><h3 id="relationship-heading" className="text-sm font-medium">Relationship</h3><p className="mt-1 text-[11px] text-[var(--color-muted-foreground)]">Induk dan semua prasyarat adalah gerbang evaluasi. Cakupan efektif ditentukan server.</p></div>{impactQuery.isFetching && <span className="text-xs text-[var(--color-muted-foreground)]">Memuat dampak…</span>}</div>{impactQuery.isError && <p role="alert" className="mt-3 text-sm text-[var(--color-destructive)]">Dampak relationship tidak dapat dimuat. Coba lagi sebelum tindakan lifecycle.</p>}{!impactQuery.isLoading && !impactQuery.isError && <div className="mt-3 space-y-3 text-sm"><RelationshipRecord label="Flag induk" flag={impact?.effective_parent} fallback={selected.parent_key || "Tidak ada"} onOpen={openFlagByKey} /><div><div className="text-xs font-medium text-[var(--color-muted-foreground)]">Prasyarat</div>{(impact?.prerequisites || []).length ? <ul className="mt-1 space-y-1">{impact!.prerequisites!.map((flag) => <li key={flag.key}><RelatedFlagButton flag={flag} onOpen={openFlagByKey} /></li>)}</ul> : <p className="mt-1 text-xs text-[var(--color-muted-foreground)]">Tidak ada prasyarat yang terselesaikan.</p>}</div><div><div className="text-xs font-medium text-[var(--color-muted-foreground)]">Dipakai langsung oleh</div>{impactDependents.length ? <ul className="mt-1 space-y-1">{impactDependents.map((dependent, index) => <li key={`${dependent.key}-${dependent.scope_id}-${index}`} className="flex items-center gap-2"><button type="button" onClick={() => openFlagByKey(dependent.key)} className="font-mono text-xs text-[var(--color-primary)] hover:underline">{dependent.key}</button><span className="text-xs text-[var(--color-muted-foreground)]">sebagai {dependent.relationship === "parent" ? "induk" : "prasyarat"} · {scopeLabel(dependent.scope_type, dependent.scope_id)}</span></li>)}</ul> : <p className="mt-1 text-xs text-[var(--color-muted-foreground)]">Belum ada flag yang bergantung langsung pada flag ini.</p>}</div></div>}<details className="mt-3"><summary className="cursor-pointer text-sm font-medium">Ubah relationship</summary><div className="mt-3 grid gap-3"><div><label htmlFor="edit-parent" className="text-xs font-medium text-[var(--color-muted-foreground)]">Flag induk</label><select id="edit-parent" disabled={!!selected.archived} className="mt-1 w-full rounded-md border bg-transparent px-2 py-2 text-sm" value={editParent} onChange={(event) => { setEditParent(event.target.value); setEditPrerequisites((current) => current.filter((item) => item !== event.target.value)); setRelationshipError(null); }}><option value="">Tanpa flag induk</option>{flags.filter((flag) => flag.key !== selected.key).map((flag) => <option key={flag.key} value={flag.key}>{flag.key}</option>)}</select></div><div><label htmlFor="edit-prerequisites" className="text-xs font-medium text-[var(--color-muted-foreground)]">Prasyarat</label><select id="edit-prerequisites" multiple disabled={!!selected.archived} size={Math.min(5, Math.max(3, relatedOptions.length))} className="mt-1 w-full rounded-md border bg-transparent px-2 py-1 text-sm" value={editPrerequisites} onChange={(event) => { setEditPrerequisites(Array.from(event.currentTarget.selectedOptions, (option) => option.value)); setRelationshipError(null); }}>{relatedOptions.map((flag) => <option key={flag.key} value={flag.key}>{flag.key}</option>)}</select></div>{relationshipError && <p role="alert" className="text-sm text-[var(--color-destructive)]">{relationshipError}</p>}<div><Button size="sm" onClick={() => validateEdit() && relationshipMut.mutate({ flagKey: selected.key, parent_key: editParent || null, prerequisites: editPrerequisites })} disabled={selected.archived || relationshipMut.isPending}>Simpan relationship</Button></div></div></details></section>
          <details className="rounded-md border border-[var(--color-warning)]/35 bg-[var(--color-warning)]/5 p-3"><summary className="cursor-pointer text-sm font-medium">Keamanan & lifecycle</summary><div className="mt-3 space-y-3"><p className="text-xs text-[var(--color-muted-foreground)]">Tindakan lifecycle menggunakan pemeriksaan dampak server. Flag yang dipulihkan kembali dalam keadaan nonaktif.</p>{impactQuery.isLoading && <p className="text-sm text-[var(--color-muted-foreground)]">Memuat dampak dependensi…</p>}{impactQuery.isError && <p role="alert" className="text-sm text-[var(--color-destructive)]">Tidak dapat memeriksa dampak dependensi. Coba lagi sebelum melanjutkan.</p>}{impactDependents.length > 0 && <div role="alert" className="rounded-md border border-[var(--color-warning)]/50 p-2 text-xs text-[var(--color-warning)]"><strong>{impactDependents.length} flag bergantung langsung pada flag ini.</strong> Arsip dan hapus permanen akan diblokir sampai relasinya dilepas. Hentikan darurat juga dapat menghentikan evaluasi mereka.</div>}<div className="flex flex-wrap items-center justify-between gap-3"><div><div className="text-sm font-medium">Hentikan darurat</div><p className="text-xs text-[var(--color-muted-foreground)]">Konsekuensi dominan: flag selalu nonaktif, mengesampingkan environment dan rollout.</p></div><Switch checked={!!selected.kill_switch} disabled={!!selected.archived} aria-label={`Hentikan darurat ${selected.key}`} onChange={(value) => value ? setKillPending(selected.key) : toggleMut.mutate({ k: selected.key, patch: { kill_switch: false } })} /></div>{killPending === selected.key && <div role="alert" className="rounded-md border border-[var(--color-warning)]/50 bg-[var(--color-warning)]/10 p-3"><p className="text-sm font-medium">Aktifkan Hentikan darurat?</p><p className="mt-1 text-xs text-[var(--color-muted-foreground)]">{impactDependents.length ? `Flag ini dipakai oleh ${impactDependents.length} flag; evaluasi terkait juga dapat terhenti.` : "Evaluasi flag akan langsung dihentikan untuk semua environment."}</p><div className="mt-2 flex gap-2"><Button size="sm" variant="outline" onClick={() => setKillPending(null)}>Batal</Button><Button size="sm" onClick={() => { toggleMut.mutate({ k: selected.key, patch: { kill_switch: true } }); setKillPending(null); }}>Hentikan darurat</Button></div></div>}<div className="flex flex-wrap gap-2 border-t border-[var(--color-border)] pt-3">{selected.archived ? <><Button size="sm" onClick={() => restoreMut.mutate(selected.key)} disabled={restoreMut.isPending}>Pulihkan dari arsip</Button><Button size="sm" variant="ghost" className="text-[var(--color-destructive)]" onClick={() => { setDeleteKey(selected.key); setDeleteTypedKey(""); }}><Trash className="h-3.5 w-3.5" /> Hapus permanen</Button></> : <Button size="sm" variant="outline" onClick={() => setArchiveKey(selected.key)} disabled={archiveMut.isPending || impactQuery.isLoading}>Arsipkan flag</Button>}</div></div></details>
        </div>}
        {panelTab === "audit" && <div><h4 className="pt-3 text-sm font-medium">Riwayat perubahan</h4>{flagAuditQuery.isLoading && <p className="pt-3 text-sm text-[var(--color-muted-foreground)]">Memuat riwayat perubahan flag…</p>}{flagAuditQuery.isError && <p role="alert" className="pt-3 text-sm text-[var(--color-destructive)]">Riwayat perubahan flag tidak dapat dimuat. Coba lagi.</p>}{!flagAuditQuery.isLoading && !flagAuditQuery.isError && <AuditTimeline entries={selectedAudit} emptyCopy="Belum ada riwayat perubahan untuk flag ini." />}</div>}
        {panelTab === "preview" && <div className="space-y-3 pt-3"><p className="text-sm text-[var(--color-muted-foreground)]">Pratinjau ini tidak mengubah state flag. Evaluasi mengikuti environment dan user yang dipilih.</p><div className="flex flex-wrap gap-2"><div className="min-w-36"><label htmlFor="preview-env" className="mb-1 block text-xs text-[var(--color-muted-foreground)]">Environment</label><select id="preview-env" className="w-full rounded-md border bg-transparent px-2 py-2 text-sm" value={previewEnv} onChange={(event) => setPreviewEnv(event.target.value)}>{ENVS.map((env) => <option key={env} value={env}>{env}</option>)}</select></div><div className="flex-1"><label htmlFor="preview-user" className="mb-1 block text-xs text-[var(--color-muted-foreground)]">User (opsional)</label><Input id="preview-user" value={previewUser} onChange={(event) => setPreviewUser(event.target.value)} placeholder="Gunakan user Anda bila kosong" /></div></div><Button size="sm" onClick={() => previewMut.mutate({ key: selected.key, env: previewEnv, user: previewUser })} disabled={previewMut.isPending}>Evaluasi pratinjau</Button>{previewResult && <div className="space-y-3 rounded-md border border-[var(--color-border)] p-3"><div><div className="flex items-center gap-2"><Badge variant={evaluationStatusLabel(previewResult) === "Aktif" ? "success" : "destructive"}>{evaluationStatusLabel(previewResult)}</Badge><span className="text-sm font-medium">{evaluationReasonLabel(previewResult.reason)}</span></div>{evaluationReasonExplanation(previewResult.reason) && <p className="mt-1 text-xs text-[var(--color-warning)]">{evaluationReasonExplanation(previewResult.reason)}</p>}<p className="mt-2 text-xs text-[var(--color-muted-foreground)]">Sumber: {readableSource(previewResult.source)} · Cakupan: {readableScopeValue(previewResult.matched_scope)}</p>{previewResult.requires && <p className="mt-1 text-xs text-[var(--color-warning)]">Prasyarat yang perlu dipenuhi: {previewResult.requires}</p>}</div><div><h4 className="text-xs font-medium text-[var(--color-muted-foreground)]">Jejak gerbang evaluasi</h4>{(previewResult.trace || []).length ? <ol className="mt-2 space-y-1">{previewResult.trace!.map((node, index) => <li key={`${node.key}-${node.relationship}-${index}`} className="flex flex-wrap items-center gap-2 rounded border border-[var(--color-border)] px-2 py-1.5 text-xs"><span className="font-mono text-[var(--color-muted-foreground)]">{index + 1}</span>{node.key && flags.some((flag) => flag.key === node.key) ? <button type="button" className="font-mono text-[var(--color-primary)] hover:underline" onClick={() => openFlagByKey(node.key!)}>{node.key}</button> : <code>{node.key || "Flag tidak diketahui"}</code>}<span>{node.relationship === "parent" ? "cek induk" : node.relationship === "prerequisite" ? "cek prasyarat" : "flag tujuan"}</span><span className="text-[var(--color-muted-foreground)]">{node.scope ? readableScopeValue(node.scope) : "—"}</span></li>)}</ol> : <p className="mt-2 text-xs text-[var(--color-muted-foreground)]">Jejak evaluasi tidak tersedia.</p>}</div></div>}</div>}
      </div>}
    </Drawer>
    {deleteKey && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4"><div role="dialog" aria-modal="true" aria-labelledby="delete-type-title" className="w-full max-w-md rounded-md border border-[var(--color-border)] bg-[var(--color-card)] p-5 shadow-[var(--shadow-popover)]"><h2 id="delete-type-title" className="text-base font-semibold">Verifikasi penghapusan permanen</h2><p className="mt-1 text-sm text-[var(--color-muted-foreground)]">Ketik tepat <code className="font-mono">{deleteKey}</code> untuk melanjutkan ke konfirmasi akhir. Flag harus sudah diarsipkan dan tidak boleh memiliki dependensi.</p><label htmlFor="delete-typed-key" className="mt-3 block text-xs font-medium text-[var(--color-muted-foreground)]">Key flag</label><Input id="delete-typed-key" className="mt-1" value={deleteTypedKey} onChange={(event) => setDeleteTypedKey(event.target.value)} autoComplete="off" /><div className="mt-4 flex justify-end gap-2"><Button variant="ghost" onClick={() => { setDeleteKey(null); setDeleteTypedKey(""); }}>Batal</Button><Button variant="destructive" disabled={deleteTypedKey !== deleteKey} onClick={() => setDeleteConfirmKey(deleteKey)}>Lanjutkan</Button></div></div></div>}
    <ConfirmDialog open={!!archiveKey} title="Arsipkan feature flag?" description={<span>Flag <code className="font-mono">{archiveKey}</code> akan dinonaktifkan. {impactDependents.length ? `${impactDependents.length} flag masih bergantung padanya; server akan memblokir pengarsipan sampai relationship dilepas.` : "Tidak ada dependent langsung yang terdeteksi."}</span>} confirmLabel="Arsipkan flag" cancelLabel="Batal" variant="destructive" busyLabel="Mengarsipkan…" busy={archiveMut.isPending} onConfirm={() => archiveKey && archiveMut.mutate(archiveKey)} onCancel={() => setArchiveKey(null)} />
    <ConfirmDialog open={!!deleteConfirmKey} title="Hapus feature flag secara permanen?" description={<span>Flag <code className="font-mono">{deleteConfirmKey}</code> dan konfigurasi registry terkait akan dihapus permanen. Server tetap memeriksa status arsip dan dependensi.</span>} confirmLabel="Hapus permanen" cancelLabel="Batal" variant="destructive" busyLabel="Menghapus…" busy={deleteMut.isPending} onConfirm={() => deleteConfirmKey && deleteMut.mutate(deleteConfirmKey)} onCancel={() => setDeleteConfirmKey(null)} />
    <ConfirmDialog open={!!rollbackKey} title="Kembalikan konfigurasi sebelumnya?" description={`Konfigurasi flag "${rollbackKey}" akan dikembalikan ke versi sebelumnya dan membuat entri audit baru.`} confirmLabel="Kembalikan konfigurasi" cancelLabel="Batal" busyLabel="Memproses…" busy={rollbackMut.isPending} onConfirm={() => rollbackKey && rollbackMut.mutate(rollbackKey)} onCancel={() => setRollbackKey(null)} />
  </div>;
}

function RelatedFlagButton({ flag, onOpen }: { flag: FlagType; onOpen: (key: string) => void }) {
  return <div className="flex flex-wrap items-center gap-2"><button type="button" className="font-mono text-xs text-[var(--color-primary)] hover:underline" onClick={() => onOpen(flag.key)}>{flag.key}</button><Badge>{statusLabel(flag)}</Badge><span className="text-xs text-[var(--color-muted-foreground)]">{scopeLabel(flag.scope_type, flag.scope_id, flag.scope_name, flag.project_name, flag.organization_name)}</span></div>;
}
function RelationshipRecord({ label, flag, fallback, onOpen }: { label: string; flag?: FlagType | null; fallback: string; onOpen: (key: string) => void }) {
