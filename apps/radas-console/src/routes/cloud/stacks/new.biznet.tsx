...
export const Route = createFileRoute("/cloud/stacks/new/biznet")({
  component: BiznetWizard,
...
function BiznetWizard() {
  ...
  const { data: schema, error: schemaError } = useQuery({
    queryKey: qk.stackSchema("biznet"),
    queryFn: () => api<Schema>("GET", "/api/cloud/biznet/schema"),
  });
  ...
  const saveMut = useMutation({
    mutationFn: async () => {
      ...
      return api("POST", "/api/cloud/stacks", { provider: "biznet", name: stackName, values: payload });
    },
    onSuccess: () => {
  ...
  if (!schema) {
    return (
      <div className="space-y-6">
        <Breadcrumbs items={[{ label: isEdit ? `Edit · ${edit}` : "New Biznet Gio Stack" }]} />
  ...
  return (
    <div className="space-y-6">
      <Breadcrumbs items={[{ label: isEdit ? `Edit · ${edit}` : "New Biznet Gio Stack" }]} />
      <div>
        <h1 className="text-2xl font-bold">{isEdit ? "Edit Stack" : "New Biznet Gio Stack"}</h1>
