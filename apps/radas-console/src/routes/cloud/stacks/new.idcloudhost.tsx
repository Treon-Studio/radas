...
export const Route = createFileRoute("/cloud/stacks/new/idcloudhost")({
  component: IdcloudhostWizard,
...
function IdcloudhostWizard() {
  ...
  const { data: schema, error: schemaError } = useQuery({
    queryKey: qk.stackSchema("idcloudhost"),
    queryFn: () => api<Schema>("GET", "/api/cloud/idcloudhost/schema"),
  });
  ...
  const saveMut = useMutation({
    mutationFn: async () => {
      ...
      return api("POST", "/api/cloud/stacks", { provider: "idcloudhost", name: stackName, values: payload });
    },
    onSuccess: () => {
  ...
  if (!schema) {
    return (
      <div className="space-y-6">
        <Breadcrumbs items={[{ label: isEdit ? `Edit · ${edit}` : "New IDCloudHost Stack" }]} />
  ...
  return (
    <div className="space-y-6">
      <Breadcrumbs items={[{ label: isEdit ? `Edit · ${edit}` : "New IDCloudHost Stack" }]} />
      <div>
        <h1 className="text-2xl font-bold">{isEdit ? "Edit Stack" : "New IDCloudHost Stack"}</h1>
